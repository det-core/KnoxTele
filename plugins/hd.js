    ...
    execute: async (sock, m, text, args) => {
        ...
        if (!buffer) {
            return newsletter.sendText(sock, m.chat, 
                '*HD CONVERT*\n\nPlease reply to an image with .hd', m
            )
        }
        
        await newsletter.sendText(sock, m.chat, '*HD CONVERT*\n\nProcessing image... This may take a moment', m)
        
        const tempDir = path.join(process.cwd(), 'temp')
        if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true })
        
        const tempFile = path.join(tempDir, `hd_${randomBytes(4).toString('hex')}.jpg`)
        ...
        try {
            ...
            while (result.status === 'waiting' && attempts < 20) {
            ...
            await newsletter.sendImage(sock, m.chat, enhancedBuffer, '*HD CONVERT*\n\nImage enhanced successfully!', m)
