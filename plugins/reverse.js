...
export default {
    ...
    execute: async (sock, m, text, args) => {
        ...
        const inputPath = path.join(tempDir, `reverse_in_${randomBytes(4).toString('hex')}.${ext}`)
        const outputPath = path.join(tempDir, `reverse_out_${randomBytes(4).toString('hex')}.mp3`)
        
        try {
            fs.writeFileSync(inputPath, buffer)
            
            await execAsync(`ffmpeg -y -i "${inputPath}" -af "areverse" -vn "${outputPath}"`)
