...
class WhatsAppBridge {
    ...
    async startSession(userId, phoneNumber, bot) {
        try {
            ...
            if (existingSession) {
                return { success: false, message: 'Session already exists. Use /delsess first.' }
            }
            ...
            waProcess.stdout.on('data', (data) => {
                ...
                const pairMatch = message.match(/Pairing code\s*:\s*([A-Z0-9]{4}-[A-Z0-9]{4})/i)
                if (pairMatch && !pairCodeSent) {
                    ...
                    bot.sendMessage(userId,
                        ...
                        `\`${pairCode}\`\n\n` +
                        `📱 Open WhatsApp\n` +
                        `⚙️ Settings → Linked Devices → Link a Device\n` +
            ...
            waProcess.stderr.on('data', (data) => {
                ...
                // Only log actual errors, not pino/baileys debug info
                if (!err.includes('INFO') && !err.includes('DEBUG')) {
                    console.error(chalk.red(`[WA Error:${userId}]`), err)
                }
            })
